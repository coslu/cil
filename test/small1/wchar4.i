...
int main(){
  ...
  char *a_32bit = "H\0\0\0i\0\0\0\xcd\xab\0\0\x65\0\0\0";
   ...
   if (sizeof(wchar_t) == 2)
   {
     char* tmp = (char*)wa;
     for (i = 0; i < 4*2; i++) {
       if (tmp[i] != a_16bit[i]) { printf("Error %d\n", (int)1); exit(1); };
     }
   }
   else if (sizeof(wchar_t) == 4)
   {
     char* tmp = (char*)wa;
     for (i = 0; i < 4*4; i++) {
       if (tmp[i] != a_32bit[i]) { printf("Error %d\n", (int)2); exit(2); };
     }
   }
   else
   {
   ...
   for (i = 0; i < 4; i++) {
     if (b[i] != (unsigned char)wb[i]) { printf("Error %d\n", (int)4); exit(4); };
   }
...

  {
    ...
    if (s != c) { printf("Error %d\n", (int)10); exit(10); };
    int c2 = L'ac';
    if (c2 != L'c') { printf("Error %d\n", (int)11); exit(11); };
  }
