...
int main()
{
  wchar_t aa[] = L"A\xabcd";
  wchar_t ba[] = L"A\\xabcd";
  wchar_t *ap = L"A\xabcd";
  wchar_t *bp = L"A\\xabcd";
  char *p1, *p2;
  int i;

  if (wcslen(aa) != wcslen(ap)) { { printf("Error %d\n", (int)1); exit(1); }; }
  if (wcslen(ba) != wcslen(bp)) { { printf("Error %d\n", (int)2); exit(2); }; }
  if (wcslen(aa) == wcslen(ba)) { { printf("Error %d\n", (int)3); exit(3); }; }
  if (wcslen(ap) == wcslen(bp)) { { printf("Error %d\n", (int)4); exit(4); }; }

  p1 = aa;
  p2 = ap;
  for (i=0; i<2 * sizeof(wchar_t); i++) {
    if (p1[i] != p2[i]) { { printf("Error %d\n", (int)5); exit(5); }; }
  }
  p1 = ba;
  p2 = bp;
  ...
  if (wcscmp(aa,ap) != 0) { { printf("Error %d\n", (int)7); exit(7); }; }
  if (wcscmp(ba,bp) != 0) { { printf("Error %d\n", (int)8); exit(8); }; }
  if (wcscmp(aa,ba) == 0) { { printf("Error %d\n", (int)9); exit(9); }; }
  if (wcscmp(ap,bp) == 0) { { printf("Error %d\n", (int)10); exit(10); }; }
